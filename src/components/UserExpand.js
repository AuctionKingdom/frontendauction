          ...
          return Object.keys(props.playerList).map((key)=>{
              if(JSON.parse(props.playerList[key])['highestBidder'] === emailId){
                  return(
                    <ListItem>
                        <ListItemIcon>
                              <img src={JSON.parse(props.playerList[key])['imgsrc']} alt="Player" style={{height:30,width:30,borderRadius:50}} />
                        </ListItemIcon>
                        <ListItemText primary={JSON.parse(props.playerList[key])['player']} />
                    </ListItem>
                  )
              }else{
                  return(
